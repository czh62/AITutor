...
/**
 * LabelsSelect择下拉框：Select择要Query的实体Labels（* 表示全局图谱）。
 * 自Includes异步 combobox（不Depends On react-select / AsyncSelect）。
 */
const GraphLabels = () => {
  ...
  // 拉取Select项列表：空Query用热门Labels，否则后端搜索
        ...
        <Popover open={open} onOpenChange={setOpen}>
          ...
          >
            ...
            <div className="max-h-72 overflow-auto">
              {options.length === 0 ? (
                <div className="py-6 text-center text-sm text-gray-500">No labels</div>
              ) : (
                options.map((opt) => (
                  <div
                    key={opt}
